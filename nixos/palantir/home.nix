...

{
  ...
  home.activation.appBlacklist = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
  ...
  # current profile.
  home.activation.clearKSycoca = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
  ...
  home.activation.kwinEisNoPrompt = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    $DRY_RUN_CMD ${pkgs.kdePackages.kconfig}/bin/kwriteconfig6 \
      --file kwinrc \
      --group Xwayland \
